...
public interface PermissionRepository extends JpaRepository<GroupPermission,Long> {
    ...
    @Query(value = "select g.name,p.id,p.permission from group_permissions gp left join `groups` g on g.id = gp.group_id left join permissions p on p.id= gp.permission_id where g.id =:groupId",nativeQuery = true)
