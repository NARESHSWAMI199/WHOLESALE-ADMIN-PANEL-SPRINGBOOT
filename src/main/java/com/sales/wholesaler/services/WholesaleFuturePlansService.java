...
public class WholesaleFuturePlansService extends WholesaleRepoContainer {
    ...
    public Page<WholesalerFuturePlans> getWholesalerFuturePlans(User loggedUser, SearchFilters filters) {
    ...
    public int activateWholesalerFuturePlans(User loggedUser,String servicePlanSlug){
        ...
        List<WholesalerFuturePlans> futurePlans = wholesaleFuturePlansRepository.findWholesalerFuturePlansByServicePlan(servicePlan);
        List<WholesalerFuturePlans> plans = futurePlans.stream().filter(futurePlan -> futurePlan.getServicePlan() != null && futurePlan.getServicePlan().getSlug().equals(servicePlanSlug)).toList();
        if(!plans.isEmpty()) {
            WholesalerFuturePlans futurePlan =plans.get(0);
